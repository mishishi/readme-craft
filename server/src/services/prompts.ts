...
6. **空行分隔**：段落之间用空行隔开，营造呼吸感
7. **不要使用 emoji**：保持文字本身的简洁
8. **特性描述格式**：列表形式，每项为 "**特性名**：一句话说明"`,
...
4. **架构说明**：系统架构部分用文字代码块描述分层，不要使用 mermaid（渲染不稳定）
5. **前后端分明**：技术架构部分必须明确区分前端/后端/基础设施
6. **不要使用 emoji**：保持专业感，标题也不要用 emoji`,

  cards: `
## 专属风格规则（卡片视界）

1. **引用块（>）必须保留**：项目简介的 3 个引用块、核心特性的 5 个引用块全部保留
2. **每个特性必须用 > 引用块包裹**：不要改为普通段落或列表；每个引用块内至少 2 句话描述
...
2. **叙事风格**：「项目简介」必须使用叙事性段落书写项目的缘起、发展和愿景，而非枯燥的功能列表；每个段落控制在 3-5 句话
3. **核心特性**：使用 ### 三级标题 + 描述段落，每个特性独立；每个特性描述至少 2 句话
