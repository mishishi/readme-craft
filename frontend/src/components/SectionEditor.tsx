...
export default function SectionEditor({ section, isFirst, isLast, total, sectionIndex, onDragStart }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Drag handle */}
        <div className="flex cursor-grab items-center px-2 text-gray-300 active:cursor-grabbing" title="拖拽排序">
        ...
        <div className="flex items-center gap-0.5 pr-2">
          <button
            ...
            title="上移"
            aria-label="上移章节"
            className="rounded p-1.5 text-gray-400 transition-colors hover:bg-white hover:text-gray-600 disabled:cursor-not-allowed disabled:opacity-30"
          >
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button
            ...
            title="删除章节"
            aria-label="删除章节"
            className="rounded p-1.5 text-gray-400 transition-colors hover:bg-red-50 hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-30"
          >
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div>
          ...
          <div className="flex items-center gap-1 border-b border-gray-100 bg-gray-50/30 px-3 py-1.5">
            {TOOLS.map((tool) => (
              <button
                ...
                title={tool.title}
                className="rounded px-2 py-0.5 text-xs font-medium text-gray-500 transition-colors hover:bg-white hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400"
              >
                {tool.label}
              </button>
            ))}
          ...
          <div className="px-4 py-3">
            <textarea
              ...
              rows={8}
              tabIndex={0}
              className="input-field !border-0 !p-0 !shadow-none !ring-0 font-mono text-sm leading-relaxed resize-y min-h-[120px]"
