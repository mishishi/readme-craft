...
1. **引用块（>）必须保留**：项目概览的 3 个引用块、亮点特性的 4 个引用块全部保留
2. **每个亮点必须用 > 引用块包裹**：不要改为普通段落或列表
3. **分隔线保留**：--- 分隔线必须保留，用于区分各卡片区域
4. **标题 emoji 保留**：每个章节标题前的 emoji（✨ 🔥 🎨 ⚡ 🏗️ 📄）必须保留
5. **emoji 丰富**：亮点特性的卡片中使用 💡 等 emoji 增加视觉层次`,

  showcase: `
## 专属风格规则（项目展厅）

1. **Banner 保留**：开头的 <p align="center"> 和 <img> Banner 标签必须保留
2. **叙事风格**：「关于本项目」必须使用叙事性段落书写项目的缘起、发展和愿景，而非枯燥的功能列表
3. **核心亮点**：使用 ### 三级标题 + 描述段落，每个亮点独立
